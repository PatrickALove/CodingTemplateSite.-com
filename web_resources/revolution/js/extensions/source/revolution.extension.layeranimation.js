	  	...
	  	if (frame_index===0 && !obj.fromcurrentstate) {		  	
	  		...
	  		if (animobject != _nc) {		  		
				var old = jQuery.extend({},$to.anim,true);				
			  	...
			  	if (old["-webpkg-filter"]!==undefined) $to.anim["-webpkg-filter"] = old["-webpkg-filter"];			  	
			}
...
var newAnimObject = function(a) {
	...
	a["-webpkg-filter"] = a["-webpkg-filter"]===undefined ? "blur(0px) grayscale(0px)" : a["-webpkg-filter"];
...
var getcssParams = function(nc,level) {
	...
	if (level!="rekursive") {
		...
		obj.wan = nc.data('wan')===undefined ? parseInt(nc.css('-webpkg-transition'),0) || "none" : nc.data('wan');
...
var calcCaptionResponsive = function(nc,opt,level,responsive) {
	...
	if (!nc.hasClass("tp-splitted")) {
		...
		setTimeout(function() {
			nc.css("-webpkg-transition", nc.data('wan'));
