...
jQuery.extend(true,_R, {
	...
	manageVideoLayer : function(_nc,opt,recalled,internrecalled) {	
		...
		switch (newvideotype) {
			...
			//	if (location.protocol === 'https:')
				httpprefix = "https";												
				_nc.data('videomarkup','<iframe src="'+httpprefix+'://player.vimeo.com/video/'+vimeoid+'?autoplay=0&'+vida+'" webpkgallowfullscreen mozallowfullscreen allowfullscreen width="100%" height="100%" style="opacity:0;visibility:hidden;100%;height:100%"></iframe>');
...
var exitFullscreen = function() {
  ...
  } else if(document.webpkgExitFullscreen) {
    document.webpkgExitFullscreen();
  }
}
...
var checkfullscreenEnabled = function() {
   try{
	    ...
	    var heightMargin = 5;
	    if(jQuery.browser.webpkg && /Apple Computer/.test(navigator.vendor)) {
	...
	if (fullScreenButton!=undefined) {
		...
		if (fullScreenButton)
			addEvent(fullScreenButton,"click", function() {
				...
				} else if (video.webpkgRequestFullscreen) {
					video.webpkgRequestFullscreen(); // Chrome and Safari
				}
			});
